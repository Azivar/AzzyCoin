...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27001
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 26900
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16332
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 16335
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 27001
	%{_sbindir}/semanage port -d -p tcp 26900
	%{_sbindir}/semanage port -d -p tcp 16332
	%{_sbindir}/semanage port -d -p tcp 16335
	for selinuxvariant in %{selinux_variants}; do
